...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		## Show Welcome Message, close Notepad++ if required after 10 minute countdown, verify there is enough disk space to complete the install, and persist the prompt
		Show-InstallationWelcome -CloseApps 'notepad++' -CheckDiskSpace -PersistPrompt -CloseAppsCountdown 600
		...
		[string]$MyArguments = "-ExecutionPolicy Bypass -NoLogo -File `"$dirFiles\InstallNotePadPlusPlus.ps1`" "
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		## Show Welcome Message, close Notepad++ with a 600 second countdown before automatically closing
		Show-InstallationWelcome -CloseApps 'Notepad++' -CloseAppsCountdown 600
		...
		# Get a list of applications matching "Notepad++"
