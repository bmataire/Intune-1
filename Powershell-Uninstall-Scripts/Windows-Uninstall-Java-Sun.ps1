...
.EXAMPLE
   PS C:\> .\Windows-Uninstall-Java-Sun.ps1
...
$UninstallSearchFilter = {($_.GetValue('DisplayName') -like '*Java*') -and (($_.GetValue('Publisher') -eq 'Sun Microsystems'))}
...
Start-Sleep -Seconds 240
 ...
 ## Removes C:\Sun\Java
 if (test-path C:\Sun\Java){
    remove-item -Path c:\PerfLogs -force -recurse -Verbose -ErrorAction SilentlyContinue
} else {
    Write-Host "C:\Sun\Java does not exist, there is nothing to cleanup.                                          " -NoNewline -ForegroundColor DarkGray
